...
public class TicTacToeGUI extends JFrame {
    // Constants for maximum undos allowed and size of recent move history
    ...
    private JButton viewMovesButton;
    private JButton viewAllMovesButton;
    private JButton playAgainButton;
    private JLabel statusLabel;  // label to display the game status (e.g., current player's turn or Player X's turn)
    ...
    private void initializeGame() {
        ...
        undoCountX = MAX_UNDOS;
        undoCountO = MAX_UNDOS;
        gameOver = false;
    }
    ...
    private void initializeControlButtons(JPanel controlPanel) {
        undoButton = new JButton("Undo"); // Create Undo button
        undoButton.addActionListener(e -> handleUndo()); // Add action listener for Undo button
        controlPanel.add(undoButton); // Add Undo button to control panel

        viewMovesButton = new JButton("View Recent Moves"); // Create View Recent Moves button
        viewMovesButton.addActionListener(e -> displayRecentMoves()); // Add action listener for View Recent Moves button
        controlPanel.add(viewMovesButton); // Add View Recent Moves button to control panel

        viewAllMovesButton = new JButton("View All Moves"); // Create View All Moves button
        viewAllMovesButton.addActionListener(e -> displayAllMoves()); // Add action listener for View All Moves button
        controlPanel.add(viewAllMovesButton); // Add View All Moves button to control panel

        playAgainButton = new JButton("Play Again"); // Create Play Again button
        playAgainButton.addActionListener(e -> playAgain()); // Add action listener for Play Again button
    ...
    private void handleUndo() {
        ...
        // Check if the current player has any undos left and there are moves to undo
        if (currentPlayer == 'X' && !undoStackX.isEmpty() && undoCountX > 0) {
            undoMove(undoStackX);
            undoCountX--;
            statusLabel.setText("Undo successful. Player X has " + undoCountX + " undos left.");
        } else if (currentPlayer == 'O' && !undoStackO.isEmpty() && undoCountO > 0) {
            undoMove(undoStackO);
            undoCountO--;
            statusLabel.setText("Undo successful. Player O has " + undoCountO + " undos left.");
        } else {
            statusLabel.setText("No moves to undo or no undos left!");
        }
    }
    ...
    private void undoMove(Stack<Move> undoStack) {
        ...
        // Removing the lastmove from the Queue and Linked list
